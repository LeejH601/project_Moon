...
class Player(Object, Singleton):
    ...
    def rendering(self):
        if self.state == 'SSHIELD' or self.state == 'SHEILDWALK':
            x, y = self.locate
            w, h = self.sprites[str(self.name)+self.state+str(self.Holding_direct)][self.animation_frame].w, self.sprites[str(self.name)+self.state+str(self.Holding_direct)][self.animation_frame].h
            self.sprites[str(self.name)+self.state+str(self.Holding_direct)][self.animation_frame].draw_to_origin( x, y,w*s_size,h*s_size)
        else:
            super().rendering()
        if self.state == 'ATTACK':
            ...
            if self.equiped_weapon == 0:
                ...
                if self.animation_frame < 17:
                    ...
                    w_w, w_h = self.sprites[str(self.name)+'SSWORD'+str(self.direct)][self.animation_frame+temp].w, self.sprites[str(self.name)+'SSWORD'+str(self.direct)][self.animation_frame+temp].h
                    ...
                    self.sprites[str(self.name)+'SSWORD'+str(self.direct)][self.animation_frame+temp].draw_to_origin(x, y,w_w*s_size,w_h*s_size)
